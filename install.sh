...
echo -e "${BLUE}[*] Installing OS dependencies (Including obfs4proxy)...${NC}"
apt-get update -y
DEBIAN_FRONTEND=noninteractive apt-get install -y tor gcc build-essential curl wget sudo psmisc imagemagick xdotool obfs4proxy
...
# Removed heartbeat.c, shadownet_engine.c, and shadownet.c from migration
cp shadow_rules.js shadow_launcher.sh shadow.png shadow-browser.desktop "$INSTALL_DIR/" 2>/dev/null

cd "$INSTALL_DIR" || exit

# Compilation of Noise & Jitter Engines removed
